...
Inductive para_red : term → term → Prop :=
| para_red_var : forall (x:termvar),
  para_red (term_var_f x) (term_var_f x)
| para_red_abs : forall (L:vars) (τ: typ) (e e':term),
  (forall x , x ∉ L  → para_red  (e ^ x) (e' ^ x))  →
  para_red (term_abs τ e) (term_abs τ e')
| para_red_app1 : forall (e1 e2 e1' e2':term),
  para_red e1 e1' →
  para_red e2 e2' →
  para_red (term_app e1 e2) (term_app e1' e2')
| para_red_app2 : forall (L:vars) (τ: typ) (e1 e2 e2' e1':term),
  (forall x , x ∉ L → para_red  (e1 ^ x)  (e1' ^ x))  →
  para_red e2 e2' →
  para_red (term_app (term_abs τ e1) e2) (e1' ^^ e2').
Hint Constructors para_red.
Notation "e1 '⇒' e2" := (para_red e1 e2) (at level 68).
Notation "e1 '⇒⁺' e2" := (clos_trans para_red e1 e2) (at level 68).
...
auto.
auto.
apply lc_body_term_wrt_term; auto;
apply lc_body_term_abs_2 with (t1:=τ); eauto.
Qed.
Hint Resolve para_red_regular1 para_red_regular2.

Inductive can : term → term → Prop :=
| can_var : forall (x:termvar),
  can (term_var_f x) (term_var_f x)
| can_abs : forall (L:vars) (τ: typ) (e e':term),
  (forall x, x ∉ L → can (e ^ x) (e' ^ x)) →
  can (term_abs τ e) (term_abs τ e')
| can_app1 : forall (e1 e2 e1' e2':term),
  (forall τ e', e1 ≠ term_abs τ e')  →
  can e1 e1' →
  can e2 e2' →
  can (term_app e1 e2) (term_app e1' e2')
| can_app2 : forall (L:vars) (τ: typ) (e1 e2 e2' e1':term),
  (forall x, x ∉ L → can (e1 ^ x)  (e1' ^ x)) →
  can e2 e2' →
  can (term_app (term_abs τ e1) e2) (e1' ^^ e2') .
Hint Constructors can.
...
pick fresh x.
apply lc_body_term_wrt_term; auto;
apply lc_body_term_abs_2 with (t1:=τ); auto.
Qed.
...
Case "abs". pick fresh x; destruct (H0 x) as [e2 H2].
  exists (term_abs t (close_term_wrt_term x e2)).
...
apply para_red_abs with (L := L `union` singleton y); intros.
  ...
  assert (term_var_f x = subst_term f2' y (term_var_f x)) as H5. autorewrite with lngen; reflexivity.
  rewrite H3 at 1; clear H3.
  rewrite H5 at 2; clear H5.
  repeat rewrite <- subst_term_open_term_wrt_term; eauto.
apply para_red_app1; auto.
rewrite subst_term_open_term_wrt_term; eauto.
  assert (para_red (subst_term f2 y (term_abs τ e0)) (subst_term f2' y (term_abs τ e1'0))) as Ht.
    apply IHHlc1; auto;
    apply para_red_abs with (L := L); auto.
...
inversion Hcan; subst.
  constructor; auto.
  inversion Hpara1; subst; pick fresh x; assert (para_red (term_abs τ e') (term_abs τ e1'0)) as H by eauto.
...
apply rt_refl.
apply rt_trans with (y := term_app (term_abs τ e1') e2'); eauto.
