...
Fixpoint close_term_wrt_term_rec (n1 : nat) (x1 : termvar) (e1 : term) {struct e1} : term :=
  match e1 with
    ...
    | term_abs e2 => term_abs (close_term_wrt_term_rec (S n1) x1 e2)
...
Inductive degree_term_wrt_term : nat -> term -> Prop :=
  ...
  | degree_wrt_term_term_abs : forall n1 e1,
    degree_term_wrt_term (S n1) e1 ->
    degree_term_wrt_term n1 (term_abs e1)
...
Hint Extern 1 (lc_term (term_abs _)) =>
  let x1 := fresh in
  pick_fresh x1;
...
Hint Resolve lc_body_term_abs_1 : lngen.
...
forall x2 e2 e1 x1,
  ...
  subst_term e1 x1 (term_abs e2) = term_abs (close_term_wrt_term x2 (subst_term e1 x1 (open_term_wrt_term e2 (term_var_f x2)))).
Proof.
default_simp.
Qed.
